...
async function waitOverlayGone(page) {
  await page.locator('.window-overlay').waitFor({ state: 'hidden', timeout: 0 }).catch(() => {});
  await page.locator('.busy-load-container').waitFor({ state: 'hidden', timeout: 0 }).catch(() => {});
}
...
async function findPageWithText(ctx, text, timeout = 0) {
