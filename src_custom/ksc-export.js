...
const { APPLIED_RUNTIME_PARAMS, CONFIG, ensureDir, logStartupConfig, validateRuntimeConfig } = require('./config');
const {
...
(async () => {
  ...
  try {
    ...
    if (exportJobs.length > 0) {
      ...
      console.log('No standard export jobs configured. Skipping standard report flow.');
    }
    ...
    const runResult = finalizeRunResult({
      outputDir: CONFIG.outputDir,
      exportResults,
      multiPeriodResult,
      finalSummaryPaths,
    ...
    console.log('Run manifest file =', runResult.manifestPath);
    console.log(`RUN_RESULT_JSON=${JSON.stringify(runResult)}`);
    console.log('DONE');
  } catch (error) {
    exitCode = 1;
...
function pickFinalSummaryInputs(exportResults) {
  let dailyResult = null;
  let monthlyResult = null;
  let yearlyResult = null;

  for (let index = exportResults.length - 1; index >= 0; index -= 1) {
    const result = exportResults[index];
    const jobType = normalizeJobType(result.job);

    if (jobType === 'daily' && !dailyResult) {
      dailyResult = result;
      continue;
    }

    if (jobType === 'monthly' && !monthlyResult) {
      monthlyResult = result;
      continue;
    }

    if (jobType === 'yearly' && !yearlyResult) {
      yearlyResult = result;
    }
  }

  if (!dailyResult || !monthlyResult || !yearlyResult) {
    return null;
  }
...
function normalizeJobType(job) {
  const rawType = String(job?.jobType || job?.filePhase || '').trim().toLowerCase();

  if (rawType.includes('daily')) return 'daily';
  if (rawType.includes('monthly') || rawType.includes('mtd')) return 'monthly';
  if (rawType.includes('yearly') || rawType.includes('ytd')) return 'yearly';
